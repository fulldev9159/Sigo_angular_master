...
export class OtsPaymentComponent implements OnInit, OnDestroy {
  ...
  onGetKeyFromIntegracionData(data: object): any[] {
    const keys = !data
      ...
      : Object.keys(data).reduce(
          (arr, key) => [...arr, { header: key, field: key }],
          []
        );
    return keys;
  }
  ...
  async getImputacionData(data: any[]) {
    ...
    this.integracionDataColumn = this.onGetKeyFromIntegracionData(
      this.integracionData?.[0]
    );
  ...
  onUploadCsvFile(event: any) {
    ...
    this.integracionData.map(item => {
      if (
        act_id != Number.parseInt(item?.id_acta) ||
        ot_id != Number.parseInt(item?.id_ot)
      ) {
    ...
    postData.map(item => {
      ...
      this.integracionData.reduce((newData, curData) => {
        if (
          item.acta_id == Number.parseInt(curData.id_acta) &&
          item.ot_id == Number.parseInt(curData.id_ot)
        ) {
          let obj = this.joinItems.reduce(
            (obj, key) => ({
              ...obj,
              [key]: this.toConvertType(key, curData[key]),
            }),
            {}
          );
  ...
  onChangeCsvFile(event: any): void {
    ...
    reader.onload = () => {
      ...
      this.getImputacionData(
        this.getDataRecordsArrayFromCSVFile(csvRecordsArray, headers)
      );
    };
  ...
  getDataRecordsArrayFromCSVFile(csvRecordsArray: any[], headers: any[]) {
    ...
    for (let i = 1; i < csvRecordsArray.length; i++) {
      let curruntRecord = (<string>csvRecordsArray[i])
        .replace(/"/g, '')
        .split(';');
      if (curruntRecord.length == headers.length) {
        csvArr.push(
          headers.reduce(
            (obj, header, index) => ({
              ...obj,
              [header.replace(/"/g, '').trim()]: curruntRecord[index].trim(),
            }),
            {}
          )
        );
