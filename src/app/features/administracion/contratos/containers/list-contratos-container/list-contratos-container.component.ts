...
export class ListContratosContainerComponent implements OnInit {
  ...
  editar(item: TableListContratosMarcos): void {
    this.router.navigate(['/administracion/contratos/form-contratos', item.id]);
  }

  activacion(event: any, item: TableListContratosMarcos): void {
    this.logger.debug(`activacion triggered`);
    // const estado = item.estado === 'Activo' ? false : true;
    const txt = !item.estado ? 'activar' : 'desactivar';
    const message = `¿Está seguro que desea ${txt} este contrato?`;
    if (confirm(message)) {
      const request: ReqActivarContrato = {
        contrato_marco_id: +item.id,
        values: {
